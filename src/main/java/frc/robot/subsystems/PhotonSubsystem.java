 ...
 */
  public Command getAprilTagCommand(PhotonPositions spacePositions, CommandXboxController driverStick){
    ...
    return Commands.sequence(
      new ProxyCommand(getWaitForDataCommand(spacePositions.id)), // Proxy this command to prevent PhotonSubsystem requirement conflicting with swerveRequirementCommand
      new ScheduleCommand(new RumbleJoystick(driverStick, RumbleType.kBothRumble, 0.5, 0.4, true)),
      new ProxyCommand(moveToTargetCommands) // Proxy this command to prevent SwerveSubsystem requirement conflicting with swerveRequirementCommand
    ).finallyDo(() -> new RumbleJoystick(driverStick, RumbleType.kBothRumble, 0.5, 0.4, false).schedule());
  }
